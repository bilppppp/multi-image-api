...
当前版本已实现 Tuzi 的 `gpt-image-2` 文生图和图生图能力。ALAPI 以后可以作为新的 provider 加入，但当前仓库默认还没有启用 ALAPI。
...
- 安全处理 key：只读取环境变量，不把 key 写进命令、文件或结果里。
...
如果没有设置，停止并请用户在 agent 看不到完整命令的地方设置环境变量。不要让用户把 key 粘贴到聊天里。

4. 调用时优先使用后台模式。

```bash
bun <skill目录>/scripts/main.ts --prompt "A cat" --output "$(pwd)/cat.png" --background
```
